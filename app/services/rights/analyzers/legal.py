...
class LegalAnalyzer:
    ...
    def analyze(self, text: str, context: Dict = None) -> Dict:
        ...
        checks = [
            self._check("STATUTORY_SURFACE_RIGHT", "법정지상권", text, ["법정지상권", "지상권"], 60),
            self._check("LIEN", "유치권", text, ["유치권", "유치권 신고", "공사대금", "lien"], 70),
            self._check("PRELIMINARY_REGISTRATION", "예고등기", text, ["예고등기"], 90),
            self._check("PROVISIONAL_DISPOSITION", "가처분", text, ["가처분", "처분금지가처분"], 60),
            self._check("PROVISIONAL_REGISTRATION", "가등기", text, ["가등기", "담보가등기"], 50),
            self._check("CO_OWNERSHIP", "공유지분", text, ["공유지분", "지분매각", "공유자"], 40),
            self._check("SENIOR_LEASEHOLD", "선순위 전세권", text, ["선순위 전세권", "전세권설정", "전세권"], 50),
            self._check("DIVIDEND_DEADLINE", "배당요구종기", text, ["배당요구종기", "배당요구 종기", "배당요구기한"], 20),
        ]
